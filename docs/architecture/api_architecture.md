...
We recommend the following workflow for using the EnergyPod Calculator API:
1. Create or convert the baseload profile.
2. Create an EnergyDTO with the configuration of your company, including the baseload profile.
3. Check and update the current connection capacity.
4. Run the appropriate without/with EnergyPod endpoint to calculate the energy usage and costs over a year.
5. Calculate the yearly grid tariff, yearly savings and EnergyPod payback time.

The interaction between the endpoints when used in this order is shown in the schema below.
...
- **Vehicle information**: There are three types of vehicles that are considered: vans, box trucks and semi-trailer trucks. For each type of vehicle, provide the number of vehicles, the power usage per km and the total number of km driven per year per vehicle as `VehicleTypeInfoDTO` object. The `VehicleTypeInfoDTO` objects for each vehicle type are bundled into a `VehicleInfoDTO` object that can be provided as *vehicle information* input for the `EnergyDTO` object.
...
- **EnergyPod information**: The EnergyPod information consists of the following parameters:
    ...
    - *Battery capacity*: The battery capacity can be variable or fixed. If you want to use a pre set value for the battery capacity in the optimization, you need to fix the battery capacity by providing its value as input in the `EnergyDTO` object. This value should be in kWh. If you want to let the optimization determine the optimal battery capacity based on the energy demand and the investment costs for a battery, the battery capacity needs to be a variable with the optimization. In this case, you can set the battery capacity to be `None`.

### Baseload Profile Endpoints
The *baseload profile* endpoint group consists of two endpoints.
...
- **Endpoint for input file**: This endpoint takes a csv-file containing the baseload profile for a company following the formatting as used in these files for [standard electricity profiles](https://energiedatawijzer.nl/onderwerpen/profielen/standaardprofielen/) as input and can be used to obtain the baseload profile as a list of `BaseloadProfile` objects for a whole year. Note that the csv-file that is provided is validated to be in the correct format. If this is not the case, an empty list is returned.

### Check Current Connection Endpoints
The *check current connection* endpoint group consists of one endpoint. This endpoint takes an `EnergyDTO` object as input and checks for both the scenario with and without EnergyPod if the connection capacity provided is sufficient for the configuration, and if not, to what capacity it should be increased. In the scenario with EnergyPod, it also checks if the provided connection can be decreased. The endpoint returns an `EnergyDTOUpdate` object, which consists of two `EnergyDTO` objects, one for the scenario with EnergyPod and one without. These objects contain the updated connections.

### Without EnergyPod Endpoints
The *without EnergyPod* endpoint group consists of three endpoints for a small consumer, large consumer and a large conusmer with CLC respectively. All three endpoints take an `EnergyDTO` object as input and calculate the energy consumption for the scenario without EnergyPod and the resulting costs and returns these as output.
...
### With EnergyPod Endpoints
The *with EnergyPod* endpoint group consists of three endpoints for a small consumer, a large consumer and a large consumer with CLC respectively. All three endpoints take an `EnergyDTO` object as input and use an optimization to calculate the energy consumption by the charge points and the battery capacity and power usage. The resulting costs are derived from the results of the optimization and all results are returned as output.
...
### Grid Tariffs Endpoints
Aside from the energy usage costs that are determined in the *with and without EnergyPod* endpoint group, there are also costs related to grid tariffs. These costs can be calculated using the *grid tariffs* endpoint group. This endpoint group contains two endpoints for a small consumer and a large consumer respectively.
...
This endpoint serves to be able to compare the results of the scenarios with and without EnergyPod to one another. This endpoint takes the total yearly costs for both the with and without EnergyPod scenario as input and returns the yearly savings as output. The total yearly costs for both scenarios consist of the annual costs for energy usage, grid tariff, capacity exceedances and uncharged power.

### Payback Time Endpoint
This endpoint takes the yearly savings and the total investment costs in the scenarios with and without EnergyPod as input and returns the expected payback time on the investment. This endpoint is usually used in combination with the *yearly savings endpoint* as the yearly savings serve as input for this endpoint.
